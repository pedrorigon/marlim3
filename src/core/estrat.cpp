...
void estratificado::mapa(double dia,double alf){
 ...
 //if(alNd>1e-4 && agNd>1e-4){
    ...
    /*if(reyL<2400){
      ...
      dFdUl+=signUl*rl*coefpi2*swlNd/(2.*dia*pow(alNd,3.))*dfwldUl;
    }
    if(reyG<2400){
      double dfwgdUg;
      if(fabs(ugs)>1e-20)dfwgdUg=-signUg*(16/M_PI)*mig*swgNd/(rg*dia*ugs*ugs);
      else dfwgdUg=0.;
      dFdUg+=(-rg*coefpi2*swgNd*ugs*fabs(ugs)/(2.*dia*pow(agNd,3.))-
               signUd*(rg*coefpi3*siNd/(2.*dia))*(ugs*ugs/(alNd*pow(agNd,3.))-
         2.*uls*ugs/(pow(alNd,2.)*pow(agNd,2.))+uls*uls/(pow(alNd,3.)*agNd)))*
         dfwgdUg;
    }*/
    ...
    double termo3a=
    ugs*ugs*(dSidAl/(pow(agNd,3.)*alNd)-siNd/(pow(alNd,2.)*pow(agNd,3.))+
    3.*siNd/(pow(agNd,4.)*alNd));

    double termo3b=-2.*uls*ugs*(dSidAl/(pow(alNd,2.)*pow(agNd,2.))-
    2.*siNd/(pow(alNd,3.)*pow(agNd,2.))+2.*siNd/(pow(alNd,2.)*pow(agNd,3.)));

    double termo3c=
    uls*uls*(dSidAl/(pow(alNd,3.)*agNd)-3.*siNd/(pow(alNd,4.)*agNd)+
    siNd/(pow(alNd,3.)*pow(agNd,2.)));

    dFdAl=termo1+termo2+termo3*(termo3a+termo3b+termo3c);

    if(fabs(dFdUl-dFdUg)>1e-20)cine=-0.25*M_PI*dFdAl/(dFdUl-dFdUg);
    else cine=0.;
    ...
    multTrans=0.;
    double fator=1;
    if(dina>fator*pow(cine-velref,2.)) arr=-1;
   ...
   // if(arr!=1){
    ...
    //}
    //else{
	    if(alNd>1e-4)fatorperdaLiq=fl*swlNd*dia/(2.*rl*pow(alNd*dia*dia,2.));
	    else fatorperdaLiq=0.;
	    if(agNd>1e-4)fatorperdaGas=fg*swgNd*dia/(2.*rg*pow(agNd*dia*dia,2.));
	    else fatorperdaGas=0.;
    //}

 //double razao=dina/pow(cine-velref,2.);
 //if(razao>fator){
...
void estratificado::mapaTD(double chute){
	if(fabs(uls)>0.01 && fabs(ugs)>0.01){
	  ...
	  double hol;
	  if(chute<1e-15)hol=zbrent(hol1,hol2,fabs(ql),fabs(qg),0.0001);
	  else hol=zbrent(hol1,hol2,fabs(ql),fabs(qg),0.0001,0.01,0.01);
	  holliq=hol;
	  if(hol>0){
	    ...
	    arr=-1;
	    double ul=uls/hol;
	    if(fabs(ugs)>maxug) arr=1;
	    else if(ul*ang<0 && ul*ul>=9.82*dia*C2*cos(ang)/fl)arr=1;
	  }
	  else arr=1;
	}
	else arr=1;
...
double estratificado::equil(double dia){
  ...
  signUd*(fg*rg*coefpi3*siNd/(2.*dia))*(ugsv*ugsv/(alNd*pow(agNd,3.))-
  2.*ulsv*ugsv/(pow(alNd,2.)*pow(agNd,2.))+ulsv*ulsv/(pow(alNd,3.)*agNd))+
  9.82*(rl-rg)*sin(ang);
...
double estratificado::equil2(double ql,double qt, double eps){
  ...
  return
  -flC*rl*coefpi2*swlNd*ulsC*fabs(ulsC)/(2.*dia*pow(alNd,3.))+
...
double estratificado::equil3(double hol,double Ql,double Qg, double eps){
  ...
  return
  -flC*rl*coefpi2*swlNdv*ulsC*fabs(ulsC)/(2.*dia*pow(alNdv,3.))+
  fgC*rg*coefpi2*swgNdv*ugsC*fabs(ugsC)/(2.*dia*pow(agNdv,3.))+
  signd*(fgC*rg*coefpi2*siNdv/(2.*dia))*(ugsC*ugsC/(alNdv*pow(agNdv,3))-2.*ulsC*ugsC/(pow(alNdv,2.)*pow(agNdv,2))+
		  ulsC*ulsC/(agNdv*pow(alNdv,3)))-9.82*(rl-rg)*sin(ang);
}
...
double estratificado::criterio(double hol,double qlv,double qg,double eps){
    ...
    double termo3a=
    ugsC*ugsC*(dSidAl/(pow(agNdv,3.)*alNdv)-siNdv/(pow(alNdv,2.)*pow(agNdv,3.))+
    3.*siNdv/(pow(agNdv,4.)*alNdv));

    double termo3b=-2.*ulsC*ugsC*(dSidAl/(pow(alNdv,2.)*pow(agNdv,2.))-
    2.*siNdv/(pow(alNdv,3.)*pow(agNdv,2.))+2.*siNdv/(pow(alNdv,2.)*pow(agNdv,3.)));

    double termo3c=
    ulsC*ulsC*(dSidAl/(pow(alNdv,3.)*agNdv)-3.*siNdv/(pow(alNdv,4.)*agNdv)+
    siNdv/(pow(alNdv,3.)*pow(agNdv,2.)));
...
double  estratificado::calcC0(double x1,
 ...
 for(int j=0;j<maxit;j++){
   ...
   dx=f/df;
   rtn-=dx;
   if(fabs(dx)<epsn||fabs(f)<tol) return (qt-rtn)/(alf*qt);
...
// j=ugsN+ulsN;
 ...
 //double sinal=1.;
 //if(ang<0.)sinal=-1.;
 //if(fabs(vud)>0.05)vud=sinal*0.05;
...
if(alf1>0.0000001){
 ...
 double fonte1=dt*((-twg*siNd-twl*swlNd)-9.82*sin(ang)*area*(rg*alfc+rl*(1-alfc)));

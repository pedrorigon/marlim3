 ...
 * SisProd2.h
 *
 * Public interface of the new SisProd architecture.
 *
 * This header keeps only declarations, small value types and trivial inline
 * accessors. The behaviour (root finding, stream mixing, the fluid model, the
 * pressure march, the batch solver, the comparison and the test suite) lives in
 * src/core/SisProd.cpp, which improves encapsulation, readability and build
 * time.
 *
 * File organisation of the SisProd migration:
 *   - SisProd.h        : legacy interface (class SProd), still the production
 *                        solver, kept intact.
 *   - SisProd_old.cpp  : legacy implementation, preserved as the reference.
 *   - SisProd2.h       : this header, the new architecture interface.
 *   - SisProd.cpp      : the new architecture implementation.
 *
 * Implementation selection is controlled by the build option
 * MARLIM_USE_NEW_SISPROD (see CMakeLists.txt). While the new solver is being
 * completed the legacy implementation remains the default so every existing
 * flow and regression test keeps passing.
 */

#ifndef MARLIM_SISPROD2_H_
#define MARLIM_SISPROD2_H_

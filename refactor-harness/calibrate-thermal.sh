...
run_case() {
    local name="$1" from="$2" to="$3" expectation="$4"
    local work="$scratch/$name"
    mkdir -p "$work"
    cp "$pristine" "$work/SisProd.cpp"

    if [[ -n "$from" ]]; then
        python3 - "$work/SisProd.cpp" "$from" "$to" <<'PY'
import pathlib
import sys
...
PY
        if (( $? != 0 )) || cmp -s "$pristine" "$work/SisProd.cpp"; then
    ...
    output=$(MARLIM_THERMAL_SOURCES="$work/SisProd.cpp" \
